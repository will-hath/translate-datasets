...
async def process_api_requests_from_file(
    ...
    # load the progress log file
    completed_row_indices = get_completed_task_ids(progress_log_filepath)
    logging.debug(f"Loaded {len(completed_row_indices)} completed task IDs from {progress_log_filepath}")
                ...
                if next_request is None:
                    if not queue_of_requests_to_retry.empty():
                        ...
                        try:
                            ...
                            if request_json["metadata"]["row_index"] in completed_row_indices:
                                logging.debug(f"Skipping request {request_json['metadata']['row_index']} because it has already been processed")
                                continue
                    ...
                    if (
                        ...
                        asyncio.create_task(
                            next_request.call_api(
                                ...
                                progress_filepath=progress_log_filepath,
...
class APIRequest:
    ...
    async def call_api(
        ...
        if error:
            ...
            append_to_jsonl(data, save_filepath)
            track_progress(self, progress_filepath)
            status_tracker.num_tasks_in_progress -= 1
...
#### ADDED BY willhath
def track_progress(request, log_filepath):
    """Append a task ID to the end of a text log file."""
    row_indice = request.metadata["row_index"]
    with open(log_filepath, "a") as f:
        f.write(str(row_indice) + "\n")

def get_completed_task_ids(log_filepath):
    if not os.path.exists(log_filepath):
        return set()  # If the log file doesn't exist, return an empty set
    with open(log_filepath, "r") as log_file:
        return set(int(line.strip()) for line in log_file if line.strip())
####
